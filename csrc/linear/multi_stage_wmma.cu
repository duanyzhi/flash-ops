...
__global__ void multi_stage1_mma_half_kernel(
...
#pragma unroll
  for (int lk = 0; lk < loop_num; lk = lk+2) { // do loop for lk, lk = 0-> SA[0], lk = 1 -> SA[1]
...
at::Tensor multi_stage_wmma_forward(const at::Tensor & input, const at::Tensor & weight, const c10::optional<at::Tensor> & bias) {
