...
template <int BM, int BN, int BK>
__device__ void store2globalc(float* smemc, half* gmemc){
  // one thread load 128 num 
  const int tid = threadIdx.x + threadIdx.y * 32 + threadIdx.z * 2 * 32;
  for (int i = 0; i < BN; ++i) {
      gmemc[tid * BN + i] = __float2half(smemc[tid * BN +i]);
  }
}
...
__global__ void multi_stage_mma_half_kernel(
  ...
  half* smema_pong = smemb + BN * BK;
  half* smemb_pong = smema_pong + BM * BK;
  float* smemc = reinterpret_cast<float *>(smem); // 32768

  load_smema<BM, BN, BK>(smema, a + load_block_a_gmem_addr, K);
  load_smemb<BM, BN, BK>(smemb, b + load_block_b_gmem_addr, K);
...
#pragma unroll
    for (int ik = 0; ik < BK / WARP_K; ik += 1) {
      ...
      // if (ik == 0 && threadIdx.y == 0 && threadIdx.z ==0 && threadIdx.x == 0) {
      //   for (int i = 0; i < frag_b[0].num_elements; ++i) {
      //     printf("%f ", __half2float(frag_b[0].x[i]));
      //   }
      //   printf("\n"); 
      // }
...
#pragma unroll
        for (int n_index = 0; n_index < frag_b_num; n_index++) {
          ...
          // if (m_index == 0 && n_index == 0 && threadIdx.y == 0 && threadIdx.z == 0) {
            ...
            // nvcuda::wmma::mma_sync(frag_cc, frag_a[m_index], frag_b[n_index],
            //                        frag_cc);
            // __syncthreads();
            // if (threadIdx.y == 0 && threadIdx.z == 0 && threadIdx.x == 0) {
            //   printf("ik: %d. \n", ik);
            //   print_fragc<WMMA_M, WMMA_N, WMMA_K>(frag_c[0]);
            // }
            // __syncthreads();
            // __shared__ half debug[16*16];
            // half* data = debug;
            // if (threadIdx.y == 0 && threadIdx.z == 0 && (ik == 0 || ik == 1)) {
            //   printf("ik %d. \n", ik);
            //   print_fragb<WMMA_M, WMMA_N, WMMA_K>(frag_b[0], data);
            
            //   __syncthreads();
            //   if (tid == 0) {
            //     for (int a = 0; a < 16; ++a) {
            //       for (int b = 0; b < 16; ++b) {
            //         printf("%f, ", __half2float(data[a * 16 + b]));
            //         // printf("%f, ", data[a * 16 + b]);
            //       }
            //       printf("\n");
            //     }
            //   }
  ...
  store2globalc<BM, BN, BK>(smemc, c);
} 
...
at::Tensor multi_stage_mma_forward(const at::Tensor & input, const at::Tensor & weight, const c10::optional<at::Tensor> & bias) {
    ...
    // float* c_ptr = reinterpret_cast<__half*>(output.data_ptr());
